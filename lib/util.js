...
import {getLuxonDateTime} from "./timeline/DateUtil";
 ...
 * @param timezone - Optional timezone for conversion to DateTime
 * @returns Object in the desired type.
 * @throws Error
 */
export function convert(object, type, timezone = "utc") {
  let match;
  ...
  switch (type) {
    ...
    case "Number":
      if (isString(object) && !isNaN(Date.parse(object))) {
        return getLuxonDateTime(object, timezone);
      } else {
    ...
    case "Date":
      try {
        return convert(object, "DateTime").toJSDate();
      }
      catch(e){
        if (e instanceof TypeError) {
          throw new TypeError(
