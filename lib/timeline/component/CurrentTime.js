 ...
 */
  ...
  setOptions(options) {
    if (options) {
      ...
      util.selectiveExtend(['rtl', 'showCurrentTime', 'alignCurrentTime', 'locale', 'locales', 'timezone'], this.options, options);
    }
  }
  ...
  redraw() {
    if (this.options.showCurrentTime) {
      ...
      let now = DateTime.now().setZone(this.options.timezone);
      ...
      let title = `${locale.current} ${locale.time}: ${now.toFormat('cccc, MMMM d yyyy, H:mm:ss')}`;
   ...
   */
  setCurrentTime(time) {
    const t = getLuxonDateTime(time, this.options.timezone);
    const now = DateTime.now().setZone(this.options.timezone);
    this.offset = t.diff(now);
    this.redraw();
  }
   ...
   */
  getCurrentTime() {
    return DateTime.now().setZone(this.options.timezone);
  }
}
