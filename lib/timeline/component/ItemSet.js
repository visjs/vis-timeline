  ...
  setOptions(options) {
    if (options) {
      ...
      const fields = [
        ...
        'hide', 'snap', 'groupOrderSwap', 'showTooltips', 'tooltip', 'tooltipOnItemUpdateTime', 'groupHeightMode', 'onTimeout', 'scrollHorizontallyOnDrag'
      ];
      ...
      if (dragLeftItem) {
        ...
        this.touchParams.itemProps = itemsToDrag.map(id => {
          ...
          return {
            item,
            initialX: event.center.x,
            diffOfCursorAndStart: new Date(me.body.util.toTime(event.center.x).valueOf() - item.data.start.valueOf()),
  ...
  _onDrag(event) {
    ...
    if (this.touchParams.itemProps) {
      ...
      this.touchParams.itemProps.forEach(props => {
        // Date by Mouse CURRENT position
        let currentCursorPosAsTime =  me.body.util.toTime(event.center.x - xOffset);
        let initialStart;
        let initialEnd;
        let start;
        let end;
        ...
        const window = {
          start: this.body.range.getRange().start,
          end: this.body.range.getRange().end
        };
        if (updateTimeAllowed) {
          if (props.dragLeft) {
            ...
            if (this.options.rtl) {
              if (itemData.end) {
                end = currentCursorPosAsTime;
                // TODO: pass a Moment instead of a Date to snap(). (Breaking change)
                itemData.end = snap ? snap(end, scale, step) : end;
              }
            } else {
              if (itemData.start) {
                start = currentCursorPosAsTime;
                // TODO: pass a Moment instead of a Date to snap(). (Breaking change)
            ...
            if (itemData.start) {
              ...
              start = new Date(currentCursorPosAsTime.valueOf() - Math.abs(props.diffOfCursorAndStart.valueOf()));
        ...
        const scrollHorizontalOnDrag = this.options.scrollHorizontallyOnDrag;
        if (scrollHorizontalOnDrag && scrollHorizontalOnDrag.enabled) {
          const scrollSpeed = scrollHorizontalOnDrag.scrollSpeed || this.defaultOptions.scrollHorizontallyOnDrag.scrollSpeed;
          if (currentCursorPosAsTime.valueOf() < window.start.valueOf()) { // Going left
            const newPos = {
              start: window.start - (window.end - window.start) * scrollSpeed,
              end: window.end - (window.end - window.start) * scrollSpeed
            };
            this.body.range.setRange(newPos.start, newPos.end);
          } else if (currentCursorPosAsTime.valueOf() > window.end.valueOf()) { // Going right
            const newPos = {
              start: window.start + (window.end - window.start) * scrollSpeed,
              end: window.end + (window.end - window.start) * scrollSpeed
            };
