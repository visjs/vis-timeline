...
class ItemSet extends Component {
   ...
   *                           {boolean} editable.updateDuration
   *                              Allow change item duration by dragging the sides of the item
    ...
    if (options) {
      ...
      if ("editable" in options) {
        if (typeof options.editable === "boolean") {
          ...
          this.options.editable.updateDuration = options.editable;
  ...
  _onDragStart(event) {
    ...
    const isRangeResizeDrag = Boolean(dragLeftItem || dragRightItem);
    ...
    if (item && (item.selected || this.options.itemsAlwaysDraggable.item)) {
      if (
        ...
        !(isRangeResizeDrag && this.options.editable.updateDuration)
      ) {
        return;
      }
      ...
      if (
        ...
        !(isRangeResizeDrag && item.editable.updateDuration) &&
        !this.options.editable.overrideItems
      ) {
        return;
      }
      ...
      this.touchParams.itemProps.forEach((props) => {
        ...
        if (
          ...
          !(
            ...
            props.item.editable.updateDuration
          ) &&
        ...
        const updateDurationAllowed =
          ((this.options.editable.overrideItems ||
            selectedItem.editable == null) &&
            this.options.editable.updateDuration) ||
          (!this.options.editable.overrideItems &&
            selectedItem.editable != null &&
            selectedItem.editable.updateDuration);
        if (
          props.dragLeft || props.dragRight
            ? updateDurationAllowed
            : updateTimeAllowed
        ) {
          if (props.dragLeft) {
