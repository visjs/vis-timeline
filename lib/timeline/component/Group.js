 ...
 */
  ...
  _redrawItems(forceRestack, lastIsVisible, margin, range) {
     ...
     */
      ...
      for (const subgroup in this.subgroups) {
        if (!this.subgroups[subgroup].visible) {
          util.forEach(this.subgroups[subgroup].items, item => {
            item.hide();
          });
        }
      }
    ...
    if (Object.keys(this.subgroups).length > 0) {
      ...
      util.forEach(this.visibleItems, item => {
        if (item.data.subgroup !== undefined) {
          ...
          me.subgroups[item.data.subgroup].visible = this.subgroupVisibility[item.data.subgroup] !== undefined ? this.subgroupVisibility[item.data.subgroup] : true;
        }
      });
    }
  }
  ...
  _addToSubgroup(item, subgroupId=item.data.subgroup) {
    if (subgroupId != undefined && this.subgroups[subgroupId] === undefined) {
      this.subgroups[subgroupId] = {
        ...
        visible: this.subgroupVisibility[subgroupId] ? this.subgroupVisibility[subgroupId] : true,
