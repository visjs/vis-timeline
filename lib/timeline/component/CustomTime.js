 ...
 */
  constructor(body, options) {
    ...
    if (options && options.time != null) {
      this.customTime = getLuxonDateTime(options.time, options.timezone);
    } else {
      this.customTime = DateTime.now().setZone(options.timezone);
    }
  ...
  setOptions(options) {
    if (options) {
      ...
      util.selectiveExtend(['locale', 'locales', 'id', 'title', 'rtl', 'snap', 'timezone'], this.options, options);
    }
  }
  ...
  redraw() {
    ...
    if (title === undefined) {
      title = `${locale.time}: ${getLuxonDateTime(this.customTime, this.options.timezone).toFormat('cccc, MMMM d YYYY, H:mm:ss')}`;
   ...
   */
  setCustomTime(time) {
    this.customTime = getLuxonDateTime(time, this.options.timezone);
    this.redraw();
  }
   ...
   */
  getCustomTime() {
    return getLuxonDateTime(this.customTime, this.options.timezone);
  }
  ...
  _onDrag(event) {
    ...
    this.body.emitter.emit('timechange', {
      id: this.options.id,
      time: getLuxonDateTime(this.customTime, this.options.timezone),
      event
    });
