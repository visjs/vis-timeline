 ...
 */
  ...
  _repaintDeleteButton(anchor) {
	 ...
	 * @protected
	 */
  _repaintControlButton(anchor) {
    const editable = ((this.options.editable.overrideItems || this.editable == null) && this.options.editable.control) ||
                   (!this.options.editable.overrideItems && this.editable != null && this.editable.control);
    ...
    if (this.selected && editable && !this.dom.controlButton) {
      ...
      controlButton.title = optionsLocale.deleteSelected;

      // TODO: be able to destroy the control button
      this.hammerControlButton = new Hammer(controlButton).on('tap', event => {
        event.stopPropagation();
