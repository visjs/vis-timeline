 ...
 */
  ...
  isVisible(range) {
    ...
    if (align == 'right') {
      isVisible = (this.data.start.toMillis() > range.start) && (this.data.start.toMillis() - widthInMs < range.end);
    }
    else if (align == 'left') {
      isVisible = (this.data.start.toMillis() + widthInMs > range.start) && (this.data.start.toMillis() < range.end);
    }
    else {
      // default or 'center'
      isVisible = (this.data.start.toMillis() + widthInMs / 2 > range.start ) && (this.data.start.toMillis() - widthInMs/2 < range.end);
    }
    return isVisible;
  }
