 ...
 */
  ...
  static animationTimerTimeout = 20;
    ...
    if (options.animation) { // true or an Object
      ...
      const next = () => {
        if (!me.props.touch.dragging) {
          ...
          const changed = me._applyRange(s, e);
          ...
          if (done) {
            ...
            if (callback) { callback(anyChanged); }
          }
          else {
            // animate with as high as possible frame rate, leave some time in between
            // each to prevent the browser from blocking
            me.animationTimer = setTimeout(next, Range.animationTimerTimeout);
          }
        }
      };
