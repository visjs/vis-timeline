 ...
 */
  constructor(body, options) {
    super();
    const now = DateTime.now().setZone(options.timezone).startOf('day');
    const start = now.plus({days: -3});
    const end = now.plus({days: 3});
  ...
  setOptions(options) {
    if (options) {
      ...
      const fields = [
        ...
        'activate', 'hiddenDates', 'zoomKey', 'zoomFriction', 'rtl', 'showCurrentTime', 'rollingMode',
        'horizontalScroll', 'timezone'
      ];
  ...
  setRange(start, end, options, callback, frameCallback) {
    ...
    const me = this;
    const finalStart = start != undefined ? util.convert(start, 'DateTime').valueOf() : null;
    const finalEnd   = end != undefined   ? util.convert(end, 'DateTime').valueOf()   : null;
    ...
    if (options.animation) { // true or an Object
      ...
      const next = () => {
        if (!me.props.touch.dragging) {
          ...
          DateUtil.updateHiddenDates( me.body, me.options.hiddenDates, me.options.timezone);
      ...
      DateUtil.updateHiddenDates(this.body, this.options.hiddenDates, this.options.timezone);
      if (changed) {
        const params = {
  ...
  _applyRange(start, end) {
    let newStart = (start != null) ? util.convert(start, 'DateTime').valueOf() : this.start;
    let newEnd   = (end != null)   ? util.convert(end, 'DateTime').valueOf()   : this.end;
    const max = (this.options.max != null) ? util.convert(this.options.max, 'DateTime').valueOf() : null;
    const min = (this.options.min != null) ? util.convert(this.options.min, 'DateTime').valueOf() : null;
    let diff;
  ...
  _onPinch(event) {
    ...
    const hiddenDurationBefore = DateUtil.getHiddenDurationBefore(this.body.hiddenDates, this, centerDate, this.options.timezone);
  ...
  zoom(scale, center, delta, event) {
    ...
    const hiddenDuration = DateUtil.getHiddenDurationBetween(this.body.hiddenDates, this.start, this.end);
    const hiddenDurationBefore = DateUtil.getHiddenDurationBefore(this.body.hiddenDates, this, center, this.options.timezone);
    const hiddenDurationAfter = hiddenDuration - hiddenDurationBefore;
