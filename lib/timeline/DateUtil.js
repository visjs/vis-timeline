        ...
        if (duration >= 4 * pixelTime) {
          ...
          switch (hiddenDates[i].repeat) {
            ...
            case "yearly":
              ...
              break;
            default:
              console.warn("Wrong repeat format, allowed are: daily, weekly, monthly, yearly. Given:", hiddenDates[i].repeat);
              return;
          }
...
export function printDates(dates) {
  for (let i =0; i < dates.length; i++) {
    console.warn(i, new Date(dates[i].start),new Date(dates[i].end), dates[i].start, dates[i].end, dates[i].remove);
  }
}
