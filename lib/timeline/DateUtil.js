import { getDayOfYear, setDayOfYear } from '../date-utils.js';
        ...
        if (duration >= 4 * pixelTime) {
          ...
          switch (hiddenDates[i].repeat) {
            case "daily": // case of time
              ...
              setDayOfYear(startDate, getDayOfYear(start.toDate()));
              startDate.year(start.year());
              startDate.subtract(7,'days');

              setDayOfYear(endDate, getDayOfYear(start.toDate()));
...
export function stepOverHiddenDates(timeStep, previousTime) {
  ...
  if (stepInHidden == true && currentValue < timeStep._end.valueOf() && currentValue != previousTime) {
    ...
    else if (getDayOfYear(prevValue) != getDayOfYear(newValue)) {timeStep.switchedDay = true;}
