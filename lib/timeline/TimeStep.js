...
class TimeStep {
  ...
  roundToMinor() {
    ...
    switch (this.scale) {
      case 'year':
        ...
        this.current = this.current.month(0);                            // eslint-disable-line no-fallthrough
      ...
      case 'weekday':      this.current = this.current.hours(0);         // eslint-disable-line no-fallthrough
      case 'hour':         this.current = this.current.minutes(0);       // eslint-disable-line no-fallthrough
      case 'minute':       this.current = this.current.seconds(0);       // eslint-disable-line no-fallthrough
      case 'second':       this.current = this.current.milliseconds(0);  // eslint-disable-line no-fallthrough
  ...
  next() {
    ...
    switch (this.scale) {
      ...
      case 'week':
        if (this.current.weekday() !== 0){ // we had a month break not correlating with a week's start before
          this.current = this.current.weekday(0); // switch back to week cycles
    ...
    if (this.step != 1) {
      ...
      switch (this.scale) {
        case 'millisecond':  if(this.current.milliseconds() > 0 && this.current.milliseconds() < this.step) this.current = this.current.milliseconds(0);  break;
        case 'second':       if(this.current.seconds() > 0 && this.current.seconds() < this.step) this.current = this.current.seconds(0);  break;
        case 'minute':       if(this.current.minutes() > 0 && this.current.minutes() < this.step) this.current = this.current.minutes(0); break;
        case 'hour':         if(this.current.hours() > 0 && this.current.hours() < this.step) this.current = this.current.hours(0);  break;
        case 'weekday':      // intentional fall through
  ...
  static snap(date, scale, step) {
    ...
    if (scale == 'year') {
      ...
      clone = clone.date(0);
      clone = clone.hours(0);
      clone = clone.minutes(0);
      clone = clone.seconds(0);
      ...
      switch (step) {
        case 15:
        case 10:
          clone = clone.minutes(Math.round(clone.minutes() / 5) * 5);
          clone = clone.seconds(0);
          break;
        case 5:
      ...
      switch (step) {
        case 15:
        case 10:
          clone = clone.seconds(Math.round(clone.seconds() / 5) * 5);
          clone = clone.milliseconds(0);
          break;
        case 5:
