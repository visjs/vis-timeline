...
class TimeStep {
  ...
  constructor(start, end, minimumStep, hiddenDates, options) {
    this.dateTime = (options && options.luxonDateTime);
  ...
  roundToMinor() {
    ...
    if (this.scale === 'week') {
      newCurrent = newCurrent.set({weekDay: 0});
    }
  ...
  next() {
    ...
    if (this.step !== 1) {
      ...
      switch (this.scale) {
        ...
        case 'weekday':      // intentional fall through
        case 'day':
          if (this.current.day < this.step + 1) newCurrent = newCurrent.set({day: 0});
          break;
        case 'week':
          if (this.current.weekNumber < this.step) newCurrent = newCurrent.set({week: 0});
